...
//!
//! This currently lacks support for a [number of
//! important](crate#unsupported-features) SVG features. This is because this
//! integration was developed for examples, which only need to support enough
//! SVG to demonstrate Vello.
//!
//! However, this is also intended to be the preferred integration between Vello
//! and [usvg], so [consider contributing](https://github.com/linebender/vello) if you need a feature which is missing.
//!
//! [`render_tree_with`] is the primary entry point function, which supports
//! choosing the behaviour when [unsupported
//! features](crate#unsupported-features) are detected. In a future release
//! where there are no unsupported features, this may be phased out
//!
//! [`render_tree`] is a convenience wrapper around [`render_tree_with`] which
//! renders an indicator around not yet supported features
//!
...
pub fn render_tree_with<
    F: FnMut(&mut SceneBuilder, &usvg::Node) -> Result<(), E>,
    E,
>(
    sb: &mut SceneBuilder,
    svg: &usvg::Tree,
